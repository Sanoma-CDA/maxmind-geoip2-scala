...
class GeoPrivacy_test extends FunSuite with PropertyChecks {
  ...
  test("additiveNoise") {
    for (c <- cities) {
      val newPoints = (1 to 1000).map(i => additiveNoise(2.0, 4.0)(c))
  ...
  test("additiveNoiseMeters") {
    for (c <- cities) {
      val newPoints = (1 to 1000).map(i => additiveNoiseMeters(200, 1000)(c))
  ...
  test("discretized") {
    ...
    discretized(0)(p) shouldBe Point(1, 3)
    discretized(1)(p) shouldBe Point(1.2, 3.5)
    discretized(2)(p) shouldBe Point(1.23, 3.46)
    discretized(3)(p) shouldBe Point(1.235, 3.457)
    discretized(4)(p) shouldBe Point(1.2346, 3.4568)
  }
  ...
  test("discretized 2") {
    for (c <- cities) {
      val mLat = 1
      val mLong = 2
      val newPoints = (1 to 100000).map(i => discretized(1)(additiveNoise(mLat, mLong)(c)))
